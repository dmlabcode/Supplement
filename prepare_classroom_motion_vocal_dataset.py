...
def pair_and_interpolate_motion_data(data: Dict[str, pd.DataFrame], t_delta: timedelta = timedelta(microseconds=100000)) -> Dict[str, pd.DataFrame]:
...
def compute_subject_orientation(data: Dict[str, pd.DataFrame], lx_col: str, ly_col: str, rx_col: str, ry_col: str, o_col: str) -> None:
...
def compute_tag_2d_distance(data: Dict[str, pd.DataFrame], lx_col: str, ly_col: str, rx_col: str, ry_col: str, o_col: str) -> None:
...
def integrate_vocalization_data(data: Dict[str, pd.DataFrame], mapping: pd.DataFrame, its_dir: Path, rttm_dir : Path) -> None:
    ...
    rttm_files = list(sorted(rttm_dir.iterdir()))
    rttm_files = {int(str(rttmFile).split("_")[-1]) : rttmFile for rttmFile in rttm_files}
    ...
    for i, its_file in enumerate(its_files, 1):
        ...
        sdata["chn_vocal_ALICE"] = 0
        sdata["chf_vocal_ALICE"] = 0
        sdata["adult_vocal_ALICE"] = 0
        sdata["phonemes"] = 0
        sdata["syllables"] = 0
        sdata["words"] = 0
        ...
        segments_chn_ALICE, segments_chf_ALICE, segments_adult_ALICE = list(), list(), list()
        segmentes_utterances_ALICE = list()


        df = pd.read_csv(os.path.join(rttm_files[lena_id], 'diarization_output.rttm'), sep = " ", header=None)
        df = df.drop(df.columns[[0, 1, 2, 5, 6, 8, 9]], axis=1)
        df[4] += df[3]

        idx = df.loc[df[7] == "SPEECH"] #Only speech
        Vad = df.loc[df[7] != "SPEECH"] #No speech

        progress = tqdm(Vad.iterrows(), total=len(Vad),desc = f"Overlaps filter from {rttm_files[lena_id]}")

        for jj , row in progress: #For each row
            index = np.where((idx[3] <= row[4]) & (idx[4] >= row[3])) #Find overlaps
            
            if index[0].size > 0:
                ovlj = [np.maximum([idx.iloc[x][3] for x in index[0]], row[3]), np.minimum([idx.iloc[x][4] for x in index[0]], row[4])]
                idjs = np.argmax(ovlj[1] - ovlj[0]) 
                if ovlj[1][idjs] - ovlj[0][idjs] > 0: #Biggest overlap 
                    Vad.loc[jj, 3] = ovlj[0][idjs]
                    Vad.loc[jj, 4] = ovlj[1][idjs]
                    continue
            
            Vad.loc[jj, 7] = "none" #No overlap


        # RTTM integration
        for line in Vad.iterrows():
            start_time, end_time, spkr = [line for line in line[1]]
            start_time = timedelta(seconds=np.ceil(float(start_time) * 10.) / 10.)
            end_time = timedelta(seconds=np.floor(float(end_time) * 10.) / 10.)
            if spkr in ADULTS:
                segments_adult_ALICE.append((ref_time + start_time, ref_time + end_time))
            elif spkr == "KCHI":
                segments_chn_ALICE.append((ref_time + start_time, ref_time + end_time))
            elif spkr == "CHI":
                segments_chf_ALICE.append((ref_time + start_time, ref_time + end_time))

        # RTTM utterances
        with open (os.path.join(rttm_files[lena_id], 'ALICE_output_utterances.txt'), "r") as FILE:
            FILE = FILE.readlines()
            for line in FILE:
                time, phonemes, syllables, words = line.split("\t")
                start_time, end_time = [int (t) / 10000 for t in time.split(".")[0].split("_")[-2:]]
                words = words[:-1]
                end_time -= start_time
                start_time = timedelta(seconds=np.ceil(start_time * 10.) / 10.) + ref_time
                end_time = timedelta(seconds=np.floor(end_time * 10.) / 10.)
                end_time += start_time
            
                segmentes_utterances_ALICE.append((start_time, end_time, phonemes, syllables, words))
            mergeSort(segmentes_utterances_ALICE, 0, len(segmentes_utterances_ALICE) - 1)
        ...
        seg_id = {'chn_vocal': 0, 'chf_vocal': 0, 'adult_vocal': 0, 'chn_vocal_ALICE' : 0, 'chf_vocal_ALICE' : 0, 'adult_vocal_ALICE' : 0, 'utterances' : 0}
        count = 0
        progress = tqdm(
            sdata.iterrows(), total=len(sdata),
        ...
        for rid, row in progress:
            for segments, cname in [(segments_chn, 'chn_vocal'), (segments_chf, 'chf_vocal'), (segments_adult, 'adult_vocal'), (segments_chn_ALICE, 'chn_vocal_ALICE'), (segments_chf_ALICE, 'chf_vocal_ALICE'), (segments_adult_ALICE, 'adult_vocal_ALICE'), (segmentes_utterances_ALICE, 'utterances')]:
                ...
                if segments[seg_id[cname]][0] <= row["Time"] <= segments[seg_id[cname]][1]:
                    ...
                    elif cname == 'utterances':
                        sdata.loc[rid, 'phonemes'] = segments[seg_id[cname]][2]
                        sdata.loc[rid, 'syllables'] = segments[seg_id[cname]][3]
                        sdata.loc[rid, 'words'] = segments[seg_id[cname]][4]
                    else:
...
def prepare_classroom_motion_vocal_dataset(
        mf_path: Path, mdf_path: Path, its_dir: Path, rttm_dir : Path, o_path: Path = None
    ...
    integrate_vocalization_data(dataset, mapping, its_dir, rttm_dir)
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description="Generate the dataset of motion and vocalization data in one classroom observation (one day)")
    ...
    parser.add_argument("rttm_file_dir", type=str, help="The path to the directory where rtttm files locate")
    ...
    prepare_classroom_motion_vocal_dataset(
        ...
        Path(args.rttm_file_dir),
        Path(args.output_path)
    )

#python prepare_classroom_motion_vocal_dataset.py C:\Users\sebas\Desktop\work_hugo\ALICE_PARSER\03-16-2021\MAPPINGS\MAPPING_StarFish_2021.CSV  C:\Users\sebas\Desktop\work_hugo\ALICE_PARSER\03-16-2021\Ubisense_Data\MiamiLocation.2021-03-16_10-39-27-410.log  C:\Users\sebas\Desktop\work_hugo\ALICE_PARSER\03-16-2021\LENA_Data\ITS  C:\Users\sebas\Desktop\work_hugo\ALICE_PARSER\03-16-2021\ALICE  C:\Users\sebas\Desktop\work_hugo\ALICE_PARSER\03-16-2021\OUTPUT_EXAMPLE
